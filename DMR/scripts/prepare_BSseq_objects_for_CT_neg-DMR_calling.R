...
# BSseq objects for calling CT_neg-DMRs.
# Peter Hickey
# 2018-11-06
...
bsseq_filtered <- bsseq_filtered[, !colnames(bsseq_filtered) %in%
                                   genotype_failures]
# NOTE: Removing samples based on mCG even though this is mCT (-).
...
zero_coverage <- rowAnys(
  ...
  value = 0L)
# NOTE: I did some quick checks of how harsh this filter is.
#       prop.table(table(zero_coverage)) shows we retain 53% of loci
#       (remove 89,775,803 loci).
bsseq <- bsseq[!zero_coverage, ]
