...
# BSseq objects for calling CC_neg-DMRs.
# Peter Hickey
# 2018-11-06
...
bsseq_filtered <- bsseq_filtered[, !colnames(bsseq_filtered) %in%
                                   genotype_failures]
# NOTE: Removing samples based on mCG even though this is mCC (-).
...
#       prop.table(table(zero_coverage)) shows we retain 34% of loci
#       (remove 94,351,068 loci).
...
zero_coverage <- rowAnys(
  ...
  value = 0L)
# NOTE: I did some quick checks of how harsh this filter is.
#       prop.table(table(zero_coverage)) shows we retain 60% of loci
#       (remove 56,777,844 loci).
bsseq <- bsseq[!zero_coverage, ]
...
zero_coverage <- rowAnys(
  ...
  value = 0L)
# NOTE: I did some quick checks of how harsh this filter is.
#       prop.table(table(zero_coverage)) shows we retain 47% of loci
#       (remove 75,711,388 loci).
bsseq <- bsseq[!zero_coverage, ]
