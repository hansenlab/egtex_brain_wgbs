...
# BSseq objects for calling CC_pos-DMRs.
# Peter Hickey
# 2018-10-29
...
zero_coverage <- rowAnys(
  ...
  value = 0L)
# NOTE: I did some quick checks of how harsh this filter is.
#       prop.table(table(zero_coverage)) shows we retain 61% of loci
#       (remove 55,799,193 loci).
bsseq <- bsseq[!zero_coverage, ]
...
zero_coverage <- rowAnys(
  ...
  value = 0L)
# NOTE: I did some quick checks of how harsh this filter is.
#       prop.table(table(zero_coverage)) shows we retain 48% of loci
#       (remove 74,631,353 loci).
bsseq <- bsseq[!zero_coverage, ]
